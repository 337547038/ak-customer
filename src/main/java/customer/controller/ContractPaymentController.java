...
/**
 * 合同回款(ContractPayment)表控制层
 *
 * @author ak.design 337547038
 * @since 2025-08-09 12:55:22
 */
@Tag(name = "ContractPayment相关")
@RestController
...
public class ContractPaymentController {
    ...
    public ResponseEntity<Map<String, Object>> queryByPage(@RequestBody Map<String, Object> pages) {
        return ResponseEntity.ok(this.contractPaymentService.queryByPage(pages));
    }
    ...
    @PostMapping("get")
    public ResponseEntity<ContractPayment> queryById(@RequestBody Map<String, Integer> query) {
        return ResponseEntity.ok(this.contractPaymentService.queryById(query.get("id")));
    }

    /**
     * 新增数据
     *
     * @param contractPayment 实体
     * @return 新增结果Id
     */
    @Operation(summary ="新增数据")
    @PostMapping("save")
    public ResponseEntity<Integer> add(@RequestBody ContractPayment contractPayment) {
        ContractPayment result = contractPaymentService.insert(contractPayment);
    ...
    @PostMapping("edit")
    public ResponseEntity<Integer> edit(@RequestBody ContractPayment contractPayment) {
        return ResponseEntity.ok(this.contractPaymentService.updateById(contractPayment));
    }
    ...
    public ResponseEntity<Boolean> deleteById(@RequestBody Map<String,Object> ids) {
        ...
        return ResponseEntity.ok(this.contractPaymentService.deleteById(idList));
    }
